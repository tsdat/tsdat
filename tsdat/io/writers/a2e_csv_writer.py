...
class A2eCSVWriter(FileWriter):
    ...
    def get_metadata_header_str(dataset: xr.Dataset) -> str:
        ...
        header = f"header={len(global_attr_lines) + len(variable_attr_lines)}"
        global_metadata = "\n".join(global_attr_lines)
        variable_metadata = "\n".join(variable_attr_lines)

        return "\n".join([header, global_metadata, variable_metadata])

    @staticmethod
    def get_data_as_str(dataset: xr.Dataset) -> str:
        data_str = StringIO()
        dataset.to_dataframe().reset_index(drop=False).to_csv(
            data_str,
            date_format="%Y-%m-%d %H:%M:%S.%f",
            header=True,
            index=False,
        )
        return data_str.read()
    ...
    def write(
        ...
        for dims, ds in dataset_dim_groups.items():
            filepath = self.get_filepath(filepath, dims)
