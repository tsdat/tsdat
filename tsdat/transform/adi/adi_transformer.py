...
# We will always use the same coordinate system, input datastream, and output datastream
# name for every ADI dataset conversion, since tsdat only will allow one coordinate
# system and libtrans doesn't care what the names are.


# TODO: This class is *massive* and should be trimmed down. Recommended refactor into
# smaller classes and/or functions.
class AdiTransformer:
    def transform(
        ...
        """-----------------------------------------------------------------------------
        This function will use ADI libraries to transform one data variable to the shape
        defined for the output.
        This function will also fill out the output qc_ variable with the appropriate qc
        status from the transform algorithm.

        The output variable and output qc_ variables' data will be written in place.
        Any variable attribute values that were added by adi will be copied back to the
        output variables.

        Caller does not need to call this transform method for any 1-d variables where
        TRANS_PASSTHROUGH would apply.
        However, if there are two dimensions (say time and height), and the user only
        wants to transform one dimension (for example, time data is mapped to the input,
        but height data needs to be averaged), then you would need to call this
        transform and use TRANS_PASSTHROUGH for all the mapped dimension and a different
        transformation algorithm for any non-mapped dimensions.

        If all dimensions are mapped and caller does not call this method, then all
        input values and input qc values must be copied over to the output by the
        caller.  Also in this case the caller should add a 'source' attribute
        on the variable to explain what datastream the value came from.

        Parameters
        ----------
        variable_name: str
            The name of the variable being transformed.  It should have the same name in
            both the input and output datasets.
        input_dataset : xarray.Dataset
            An xarray dataset containing:
            1) A data variable to be transformed
            2) Zero or one qc_variable that contains qc flags for the data variable.
                The qc_ variable must have the exact same base name as the data
                variable. For example, if the data variable is named 'temperature',
                then the qc variable must be named qc_temperature.
                The qc_variable must not have any qc attributes set.  They will all be
                set by the transformer to specific bits that cannot be changed.
            3) One or more coordinate variables matching the coordinates on the data
                variable
            4) Zero or more bounds variables, one for each coordinate variable.  Bounds
                variables specify the front edge and back edge of the bins used to
                compute the input data values.  If no bounds variables are provided, ADI
                will assume each data point is a single, instantaneous value.  If bounds
                variables are not present in the input data files, if the user knows
                what the bin widths and alignments were for the input datastreams, they
                can specify these values via the width and alignment transformation
                parameters (note that these parameters are for the input datastreams,
                not coordinate system defaults).

                Bounds values must be the same units as their corresponding coordinate
                variable.  Exact values should be used instead of offsets.

            * Note that if range transform parameters were set on any datastreams, the
                xarray data must have at least 'range' amount of extra data retrieved
                before and after the day being processed (if it exists).

            * Note that the input variables should have been renamed to use the name
                from the output dataset.  So we should rename input variables to match
                their output names BEFORE we pass them to this method.

            * Note that variable dimensions should have been renamed to match their
                names in the output variable.

        output_dataset : xarray.Dataset
            An xarray dataset where the transformed data will go.
            The output dataset must contain:
            1) One or more coordinate variables with the shape of the defined output
            2) One empty data variable with the same shape as its coordinate variables.
                The transformed values will be filled in by ADI.
            3) One empty qc variable with the same shape as its coordinate variables.
                The qc flags and bit metadata attributes will be filled in by this
                function
            4) One or more bounds variables, one for each coordinate variable.  The
                bounds variables will contain the front edge and back edge of each bin
                for each output coord data point.  The bounds variable values can
                computed from the coordinate data points and the width and alignment
                transformation parameters.

                If the user does not specify bin width or alignment, then we use CENTER
                alignment by default and we compute the bid width as the median of all
                the deltas between coordinate points.

        transform_parameters : Dict

            A compressed set of transformation parameters that apply just to the
            specific data variable being transformed.  The following is the minimal set
            used for our initial release (more ADI parameters can be added later, as
            they will be supported by the back end).

            transform_parameters = {

                # Transformation_type defines the algorithm to use. This parameter
                # should be defined by the converter.
                # Valid values are:
                #     TRANS_AUTO (This will average if there are more input points than
                #         output points, otherwise, interpolate)
                #     TRANS_INTERPOLATE
                #     TRANS_SUBSAMPLE   (i.e., nearest neighbor)
                #     TRANS_BIN_AVERAGE
                #     TRANS_PASSTHROUGH (all values passed directly through from the
                #         input, no transform takes place)

                "transformation_type": {
                    "time": "TRANS_AUTO"
                },

                # Range specifies how far the transformer should look for the next good
                # value when performing subsample or interpolate transforms.
                # Range is always in same units as coord (e.g., seconds in this case).
                #  * Note that if range transform parameters are set for any
                #     datastreams, the xarray data must have at least 'range' amount of
                #     extra data retrieved before and after the day being processed
                #     (if it exists)!
                "range": {
                    "time": 1800
                },

                # Width applies only when using bin averaging, and it specifies the
                # width of the bin that was used to determine a specific point.  Width
                # is always in same units as coord.
                # Only use width if user wants to make the bin width different than the
                # delta between points (e.g., for smoothing data)
                # NOTE: You do not have to set width if you provide bounds variables on
                # the output dataset!
                "width": {
                    "time": 600
                }

                # Alignment applies only when using bin averaging, and it specifies
                # where in the bin the data point is located.  Valid values are:
                #    LEFT
                #    CENTER
                #    RIGHT
                # Default is CENTER
        ...
        # Now apply the coordinate system transform parameters to the coordinate system
        # group
    ...
    def _free_memory(self, adi_dataset: CDSGroup):
        # First we MUST walk through the object tree and detatch data pointers for all
        #   variables. We need to do this because the group delete will delete
        #   everything in the hierarchy, and we don't want to delete the data because
        #   it's being shared with xarray.
        #   * Note that we don't attach/detatch for time because we have to copy those
        #     values because of numpy datetime64
    ...
    def _create_adi_retrieved_dataset(
        ...
        --------------------------------------------------------------------------------
        """
        # Note:  We are not initializing datastream objects (_DSProc->datastreams)
        # because I don't think we need it for any of the libtrans operations

        # First create the dataset group
        dataset_group = cds3.Group.define(None, "retrieved_data")
        # TODO are there any global attributes that should be applied?  I don't think
        # so, so we'll skip for now...

        # Note:  I do not think that we need to add the CDSVarGroup objects to the
        # dataset (I couldn't see it being set when I stepped through the ADI code), so
        # we are leaving it out for now.
    ...
    def _create_adi_transformed_dataset(
        ...
        #  (note that trans datasets do not include obs groups!)
    ...
    def _add_atts_to_adi(self, xr_var: xr.DataArray, adi_obj: CDSObject):
        ...
        if adi_obj.get_name().startswith("qc_"):
            # Use a new dictionary of atts so we don't mess with the attributes for the
            # original data array
            atts = {}
            qc_atts = {}
    ...
    def _convert_qc_atts(self, xr_qc_atts: Dict):
        ...
        We are converting from condensed ACT format:
            flag_masks = 1U, 2U, 4U ;
            flag_meanings = "Value is equal to _FillValue or NaN", "Value is less than
            the valid_min.", "Value is greater than the valid_max." ;
            flag_assessments = "Bad", "Bad", "Bad" ;
    ...
    def _add_variable_to_adi(
        ...
        TODO: Do we need to add any VarTags?  I don't think so, since they are only used
              by dsproc, not libtrans.
              Vartags are:  source_ds_name, source_var_name, output_targets, and
              coordinate_system
        -----------------------------------------------------------------------------"""
        ...
        # Add the coordinate system name to a VarTag object for the variable (not sure
        # if we need this for transform)
    ...
    def _set_time_variable_data(self, xr_var: xr.DataArray, adi_var: CDSVar):
        """-----------------------------------------------------------------------------
        For time values, we actually have to create a copy.  We can't rely on the data
        pointer for time, because the times are converted into datetime64 objects for
        xarray.
        -----------------------------------------------------------------------------"""
    ...
    def _set_bounds_transform_parameters(
        ...
        # Get the bounds variable for each dimension used by our variable.  If no bounds
        # variable exists, then skip.
        ...
        for dim in xr_dataset[variable_name].dims:
            ...
            if bounds_var is not None:
                ...
                # We have to make sure that the data are converted to floats to set the
                # transform parameter properly
