from tsdat import TimeSeriesDataset, Config, run_pipeline, load_configs, apply_qc, save, load, FileFormat

# Load the data/qc config parameters
config: Config = load_configs(["data/pipeline.yml"])
run_pipeline(config);
...
save(dataset, "data/test_output_temp.nc", FileFormat.NETCDF)
dataset = load("data/test_output_temp.nc", FileFormat.NETCDF)
...
#
save(dataset, "data/test_output3.nc", FileFormat.NETCDF)

# Now save the dataset with qc values to csv with a companion metadata file
save(dataset, "data/test_output3.csv", FileFormat.CSV)
